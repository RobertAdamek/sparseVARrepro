...
*Warning: the simulations can take a significant amount of time to run. We ran them on an Intel® Core™ i9-10900K processor for approximately 544 hours in total. It also requires at least 16GB of RAM.*
1. Download our Docker image from https://drive.google.com/file/d/1dTdMzR5sf-zJgPyquiGKFTf1NsarA8ic/view?usp=sharing. This file is called "sparsevar-final.tar", and is quite large (~988MB). 
...
6. Create a container that runs the image: `docker run -v .:/home/output sparsevar-final`
    ...
    - This method runs the entire simulation. If you want to run the simulation in smaller chunks, you can also add another optional arugment, e.g.: `docker run -e SCRIPT_NAME=dgp0_size.R -v .:/home/output sparsevar-final`. This runs only the "dgp0_size.R" script. There are 10 scripts in total, see the "Docker image setup" folder. Each takes approximately between 30-74 hours to run.
...
- The "Empirical application" folder contains the data and R scripts which reproduce the plot of the empirical application.
